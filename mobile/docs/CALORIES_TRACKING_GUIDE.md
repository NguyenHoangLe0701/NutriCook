...
1. **Food Name Input với Auto-trigger Gemini** (Tính năng mới): 
   - Placeholder: "Ví dụ: Cá ngừ 200gr, 1 quả táo..."
   - **Tự động tính**: Sau 1.5 giây ngừng gõ, hệ thống tự động gọi Gemini
   - **Debouncing**: Hủy job cũ nếu người dùng gõ tiếp
   - **Smart logic**: Chỉ tự động tính khi calories chưa được nhập thủ công
   - Icon ✨ để tính lại thủ công (manual trigger)
...
2. **Nutrition Fields**:
   ...
   - Tự động điền sau khi Gemini tính xong

3. **UI States**:
   - Loading indicator: "Đang tính calories tự động..."
   - Success message: "✨ Đã tự động tính calories và dinh dưỡng!"
   - Error message: Hiển thị lỗi nếu không tính được

4. **Reset Button**:
   - OutlinedButton màu đỏ
   - Hủy autoCalculateJob đang chạy
   - Xóa tất cả fields và thông báo

**Code mẫu - Auto-trigger với debouncing**:
...
OutlinedTextField(
    ...
    onValueChange = { newValue ->
        ...
        // Auto-trigger Gemini sau khi ngừng gõ 1.5 giây
    ...
    trailingIcon = {
        // Icon ✨ để tính lại thủ công
        if (foodName.isNotEmpty() && geminiService?.isApiKeyConfigured() == true) {
            IconButton(onClick = { /* Manual trigger */ }) {
...
1. Người dùng nhập tên món ăn: "Cá ngừ 200gr"
2. Hệ thống kiểm tra điều kiện:
   - Tên >= 3 ký tự ✓
   - Calories chưa nhập thủ công ✓
   - Gemini service có sẵn ✓
3. Debounce 1.5 giây (hủy nếu người dùng gõ tiếp)
4. Gọi `geminiService.calculateNutrition()`
5. Parse kết quả và tự động điền vào các field
6. Hiển thị success message
7. Người dùng có thể chỉnh sửa nếu cần
